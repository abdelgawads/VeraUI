 ...
 *      (e.g. out/linux-unpacked/veraui, out/mac-arm64/VeraUI.app, out/win-unpacked/VeraUI.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'veraui-e2e-state-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/VeraUI.exe  or  out/win-x64-unpacked/VeraUI.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'VeraUI.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/VeraUI.app/Contents/MacOS/VeraUI  or  out/mac/VeraUI.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'VeraUI');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/veraui  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['veraui', 'VeraUI']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    VERAUI_EXTENSIONS_PATH: process.env.VERAUI_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    VERAUI_EXTENSION_STATES_FILE: process.env.VERAUI_EXTENSION_STATES_FILE || e2eStateFile,
    VERAUI_DISABLE_AUTO_UPDATE: '1',
    VERAUI_DISABLE_DEVTOOLS: '1',
    VERAUI_E2E_TEST: '1',
    VERAUI_CDP_PORT: '0',
  };
