...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'veraui-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'veraui-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      VERAUI_EXTENSIONS_PATH: emptyExtensionsDir,
      VERAUI_EXTENSION_STATES_FILE: extensionStatesFile,
