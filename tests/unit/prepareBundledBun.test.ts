...
describe('prepareBundledBun', () => {
  ...
  const originalCacheDir = process.env.VERAUI_BUN_CACHE_DIR;
  const originalVersion = process.env.VERAUI_BUN_VERSION;
  ...
  afterEach(() => {
    process.env.VERAUI_BUN_CACHE_DIR = originalCacheDir;
    process.env.VERAUI_BUN_VERSION = originalVersion;
  ...
  it('copies bundled bun from cache when cache metadata is valid', () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'veraui-bun-test-'));
    ...
    process.env.VERAUI_BUN_CACHE_DIR = cacheRoot;
    process.env.VERAUI_BUN_VERSION = version;
