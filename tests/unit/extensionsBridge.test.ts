...
describe('ActivitySnapshotBuilder', () => {
  ...
  it('returns correct totalConversations count', () => {
    const conversations = [makeConversation({ id: 'c1' }), makeConversation({ id: 'c2' })];
    vi.mocked(repo.getUserConversations).mockReturnValue({ data: conversations, total: 2, hasMore: false });
    vi.mocked(repo.getMessages).mockReturnValue({ data: [], total: 0, hasMore: false });

    const snapshot = new ActivitySnapshotBuilder(repo, taskManager).build();

    expect(snapshot.totalConversations).toBe(2);
  });

  it('excludes health-check conversations from totalConversations', () => {
    const conversations = [
  ...
  it('correctly counts running conversations from task manager status', () => {
    const conversations = [makeConversation({ id: 'c1', status: 'finished' })];
    vi.mocked(repo.getUserConversations).mockReturnValue({ data: conversations, total: 1, hasMore: false });
    vi.mocked(repo.getMessages).mockReturnValue({ data: [], total: 0, hasMore: false });
    vi.mocked(taskManager.getTask).mockReturnValue({ status: 'running' } as any);
  ...
  it('returns zero runningConversations when no tasks are active', () => {
    const conversations = [makeConversation({ id: 'c1', status: 'finished' })];
    vi.mocked(repo.getUserConversations).mockReturnValue({ data: conversations, total: 1, hasMore: false });
    vi.mocked(repo.getMessages).mockReturnValue({ data: [], total: 0, hasMore: false });
  ...
  it('groups conversations by agent backend', () => {
    const conversations = [
      makeConversation({ id: 'c1', type: 'gemini' as any }),
      makeConversation({ id: 'c2', type: 'gemini' as any }),
      makeConversation({ id: 'c3' }),
    ];
    vi.mocked(repo.getUserConversations).mockReturnValue({ data: conversations, total: 3, hasMore: false });
    vi.mocked(repo.getMessages).mockReturnValue({ data: [], total: 0, hasMore: false });
  ...
  it('maps error events to error state', () => {
    ...
    vi.mocked(repo.getMessages).mockReturnValue({ data: [errorMessage as TMessage], total: 1, hasMore: false });
  ...
  it('returns empty agents array when no conversations exist', () => {
