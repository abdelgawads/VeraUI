...
declare global {
  interface Window {
    __VeraSafeResizeObserver__?: boolean;
    __VeraResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__VeraSafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__VeraSafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__VeraResizeObserverPatched__) {
    ...
    window.__VeraResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__VeraResizeObserverPatched__) {
    ...
    console.__VeraResizeObserverPatched__ = true;
  }
};
