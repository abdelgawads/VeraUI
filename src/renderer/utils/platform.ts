...
const ASSET_PROTOCOL_PREFIX = 'vera-asset://asset/';
 ...
 * - In Electron dev / any HTTP(S)-served renderer: keep `vera-asset://` because direct `file://` is blocked.
 * - In Electron packaged / local-protocol renderers: convert `vera-asset://asset/{path}` to `file://` for reliable image loading.
 * - In a regular browser (WebUI): convert `vera-asset://asset/{path}` to `/api/ext-asset?path={encodedPath}`.
 *
