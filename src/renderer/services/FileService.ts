...
import { VERAUI_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理VeraUI时间戳后缀，返回原始文件名
export function cleanVeraUITimestamp(fileName: string): string {
  return fileName.replace(VERAUI_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(filePath: string): string {
  const fileName = filePath.split(/[\\/]/).pop() || '';
  return cleanVeraUITimestamp(fileName);
}
