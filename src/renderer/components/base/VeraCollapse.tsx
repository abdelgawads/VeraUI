 ...
 */
export interface VeraCollapseProps {
 ...
 */
export interface VeraCollapseItemProps {
 ...
 */
const VeraCollapseItem: React.FC<VeraCollapseItemProps> = ({ children }) => <>{children}</>;
VeraCollapseItem.displayName = 'VeraCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <VeraCollapse defaultActiveKey={['1']}>
 *   <VeraCollapse.Item name="1" header="面板1">
 *     内容1
 *   </VeraCollapse.Item>
 *   <VeraCollapse.Item name="2" header="面板2">
 *     内容2
 *   </VeraCollapse.Item>
 * </VeraCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <VeraCollapse accordion defaultActiveKey="1">
 *   <VeraCollapse.Item name="1" header="面板1">内容1</VeraCollapse.Item>
 *   <VeraCollapse.Item name="2" header="面板2">内容2</VeraCollapse.Item>
 * </VeraCollapse>
 *
 * // 自定义图标 / Custom icon
 * <VeraCollapse
 ...
 * ```
 */
const VeraCollapseComponent: React.FC<VeraCollapseProps> & { Item: typeof VeraCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<VeraCollapseItemProps> => {
      return React.isValidElement(child) && child.type === VeraCollapseItem;
    });
  }, [children]);
...
VeraCollapseComponent.Item = VeraCollapseItem;

export default VeraCollapseComponent;
