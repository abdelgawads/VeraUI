...
export class ConversationToolConfig {
  ...
  private useVerauiWebFetch = false;
  ...
  async initializeForConversation(authType: AuthType): Promise<void> {
    // 所有模型都使用 veraui_web_fetch 替换内置的 web_fetch
    this.useVerauiWebFetch = true;
  ...
  getConfig() {
    return {
      ...
      useVerauiWebFetch: this.useVerauiWebFetch,
  ...
  async registerCustomTools(config: Config, geminiClient: GeminiClient): Promise<void> {
    ...
    // 注册 veraui_web_fetch 工具（所有模型）
    if (this.useVerauiWebFetch) {
