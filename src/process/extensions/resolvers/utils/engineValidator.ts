...
/**
 * Current VeraUI extension API version.
 * Increment major for breaking changes, minor for new features, patch for fixes.
 */
export const VERAUI_VERSION = getVeraUIVersion();
...
/**
 * Get the VeraUI version from package.json.
 * Falls back to '0.0.0' if not available.
 */
function getVeraUIVersion(): string {
...
/**
 * Validate that an extension's engine requirements are satisfied by the current VeraUI version.
 *
 * Checks:
 * 1. engine.veraui — does the running VeraUI version satisfy the required range?
...
export function validateEngineCompatibility(extension: LoadedExtension): EngineValidationResult {
  ...
  // Check VeraUI core version compatibility
  if (engine?.veraui) {
    if (!satisfiesVersion(VERAUI_VERSION, engine.veraui)) {
      result.valid = false;
      result.issues.push(
        `Extension "${extension.manifest.name}" requires VeraUI ${engine.veraui} but current version is ${VERAUI_VERSION}`
      );
    }
  }
