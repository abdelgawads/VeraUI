...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.VERAUI_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.VERAUI_ALLOW_REMOTE || process.env.VERAUI_REMOTE);
  const hostHint = process.env.VERAUI_HOST?.trim();
