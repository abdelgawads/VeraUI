...
export async function resetPasswordCLI(username: string): Promise<void> {
  ...
  try {
    ...
    const dbPath = path.join(getDataPath(), 'veraui.db');
    ...
    if (!tableExists) {
      ...
      log.info('');
      log.info('Please run VeraUI at least once to initialize the database:');
      log.info('  veraui --webui');
      log.info('');
      log.info('Then you can reset the password using:');
      log.info('  veraui --resetpass <username>');
      process.exit(1);
    }
