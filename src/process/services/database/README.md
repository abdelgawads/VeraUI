# VeraUI Database System

本文档介绍 VeraUI 的新数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{userData}/config/veraui.db`
- **图片文件**: `{userData}/data/images/`

其中 `{userData}` 为：

- macOS: `~/Library/Application Support/VeraUI/`
- Windows: `%APPDATA%/VeraUI/`
- Linux: `~/.config/VeraUI/`
...
直接复制 `veraui.db` 和 `veraui.db-wal` 文件即可。
