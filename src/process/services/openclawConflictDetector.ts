 ...
 * as VeraUI Channels, and warns the user about the conflict.
 */
...
/**
 * Check if OpenClaw Lark channel conflicts with VeraUI credentials
 */
export function detectLarkConflict(verauiAppId: string): ConflictInfo | null {
  ...
  for (const [accountName, account] of Object.entries(feishu.accounts)) {
    if (account.enabled && account.appId === verauiAppId) {
      console.warn(
        `[OpenClawConflictDetector] Lark conflict detected: OpenClaw account "${accountName}" uses same appId: ${verauiAppId}`
      );
      return {
        platform: 'lark',
        openclawEnabled: true,
        credentialMatch: true,
        openclawCredential: verauiAppId,
      };
    }
  }
...
/**
 * Check if OpenClaw Telegram channel conflicts with VeraUI credentials
 */
export function detectTelegramConflict(verauiBotToken: string): ConflictInfo | null {
  const config = readOpenClawConfig();
...
export function getConflictResolutionSteps(platform: 'lark' | 'telegram'): string[] {
  ...
  return [
    `Detected conflict: OpenClaw ${platformName} channel is using the same credentials as VeraUI.`,
    ``,
    `This means messages are being handled by OpenClaw, not VeraUI Channels.`,
    `Switching agents in VeraUI will have no effect.`,
    ``,
