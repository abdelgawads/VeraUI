...
/**
 * Simplified build script for VeraUI
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const veraRunning = isProcessRunningWindows('VeraUI.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (veraRunning || electronRunning) {
        console.log('⚠️  Detected running VeraUI/Electron process. Attempting to close...');
        killWindowsProcesses(['VeraUI.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running VeraUI/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'VeraUI.exe');
    ...
    console.log('⚠️  Windows local build failed after VeraUI.exe was produced.');
    if (firstError) {
