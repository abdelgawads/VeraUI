...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('VeraUI.exe');
    await killProcessByName('VeraUI');
  ...
  const env = {
    ...process.env,
    VERAUI_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] VERAUI_EXTENSIONS_PATH: ${env.VERAUI_EXTENSIONS_PATH}`);
